...
pos_pub = rospy.Publisher('/pos', String, queue_size=1)
global can_grasp
global can_release
...
def keyboardLoop():
    ...
    #初始化监听键盘按钮时间间隔
    rate = rospy.Rate(rospy.get_param('~hz', 20))# init 10
    ...
    yaw_rate_run_ = rospy.get_param('yaw_rate_run', 3.0)#init 1.5
    # walk_vel_前后速度
    max_tv = walk_vel_
    # yaw_rate_旋转速度
    max_rv = yaw_rate_
    ...
    print "使用[WASD]控制机器人"
    print "press [g] to 2 level grasp， press[t] to 3 level grasp，press[h] to put down cube"
    print "press[p] to reset，press [r] for quit" 
